...
def make_predict_offnet_rf():
    ...
    function(input_data,path_py){
      ...
      # Updated for retrained model by: Shalini Purwar
      ...
      ##########################################################################################
      #path_py <- "E:/GIT_OPTIMUS_MASTER/optimus-machine-learning/feasibility/feasibility_offnet_wireless"
      setwd(path_py)
      ...
      tryCatch(
        {
          ...
          colnames(input_data)[colnames(input_data) == 'product_name'] <- 'Product_Name'
      ...
      #*************** File names *************************************
      model_object_name <- "Offnet_RF_mtry_7_ntree_500_selected.RData"
      youden_cutoff <-0.88
      ...
      tryCatch(
        {
          ...
          # Column Mapping
          pros_cols <- c("Provider_Name","FeasibilityResponse..Feasibility.Response.Auto.No.","Onnet.Offnet","Wireline.or.RF","Status","CustomerLatitude1","CustomerLongitude1","feasibility_response_created_date","resp_state", "Request_no","L1.or.L2","Response_related_to","BW_mbps","Final_Provider_Name","Feasibility_flag")
      ...
      tryCatch(
        {
          ...
          # Column Mapping
          cust_cols <- c("TCL_SERVICE_ID","Commissioning_Date","BW_In_MB","accuracy_num","Longitude","Latitude")
      ...
      tryCatch(
        {
          ...
          mast_ht_data_all = dbSendQuery(mydb_abstract_db, "select * from MastHeight_Data")
          # Fetching data
          mast_ht_data_all = fetch(mast_ht_data_all, n=-1)
          
          mast_ht_data_all <- mast_ht_data_all %>% 
            select(CustomerLatitude1,
                   CustomerLongitude1,
                   prospect_mast_ht) %>% dplyr::rename(prospect_lat = CustomerLatitude1,
                                                       prospect_lon = CustomerLongitude1)
          
          prospect_coords_new <- mast_ht_data_all[c("prospect_lat",
                                                    "prospect_lon",
                                                    "prospect_mast_ht")]
          prospect_coords_2 <- prospect_coords_new
          rm(mast_ht_data_all)
      ...
      fn_mast_ht_prospects_offnet_rf <- function(ID){
        ...
        TMP <- subset(input_data,input_data$Prospect_ID == ID)
        prospect_dup_coords_sub <- prospect_coords_new
        if(nrow(prospect_dup_coords_sub)==0)
      ...
      #     )
          ...
          ##########################################################################################
          # Score the inputs
          train_data_sample <- readRDS("offnet_rf_one_row_train.rds")
          scoring_data <- rbind(train_data_sample,input_data[,which(colnames(input_data) %in% colnames(train_data_sample))])[2:(nrow(input_data)+1),]
          input_data$Probabililty_Access_Feasibility <- predict(rf_model, newdata = scoring_data, type = "prob")[,2]
	  ...
	  # Business Rules
	  #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	  PROVIDER_BTS_DIST_MAX <- 4
	  
	  input_data$Prediction_Reason <- ifelse(input_data$Predicted_Access_Feasibility == "Feasible",
											 "Access Feasible",
											 "Access Infeasible")
	  #a) BTS distance > max
	  input_data$Prediction_Reason <- ifelse(input_data$Predicted_Access_Feasibility == "Feasible" & input_data$provider_min_dist > PROVIDER_BTS_DIST_MAX,
											 paste0("Privider BTS DIST > MAX(",PROVIDER_BTS_DIST_MAX,"km)"),
											 input_data$Prediction_Reason)
	  input_data$Predicted_Access_Feasibility <- ifelse(input_data$provider_min_dist > PROVIDER_BTS_DIST_MAX,
														"Not Feasible",
														input_data$Predicted_Access_Feasibility)
	  #b) Mast Height > max
	  MAST_HT_MAX_IN_M <- 9
	  
	  input_data$Predicted_Access_Feasibility <- ifelse(input_data$Predicted_Access_Feasibility == "Feasible",
											ifelse(input_data$avg_mast_ht > MAST_HT_MAX_IN_M,
												   "Not Feasible",
												   input_data$Predicted_Access_Feasibility),
											input_data$Predicted_Access_Feasibility)
	  input_data$Prediction_Reason <- ifelse(input_data$Predicted_Access_Feasibility == "Feasible",
											 ifelse(input_data$avg_mast_ht_cost > MAST_HT_MAX_IN_M,
      ...
      ######################**** MACD CODE BEGINS ******#################################################
      ## Attribute Added for MACD: Non FE/ GE interface
      input_data$Predicted_Feasibility_Comment <- "NA"
      ###########**** MACD CODE ENDS ******############################################
