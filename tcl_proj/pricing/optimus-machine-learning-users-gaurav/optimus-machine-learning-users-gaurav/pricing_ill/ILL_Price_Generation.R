run_ILL_pricing <- function(bm_data, path_py){
  ...
  ##########################################################################################
  #1. Set path
  #path_py <- "E:/Project_Codes/ProjectOptimus/Codes/Pricing/ILL_Prod_Code_V8"
  setwd(path_py)
  ...
  if(ETL_CONNECTION){
    tryCatch(
      {
        oppy_with_products = dbSendQuery(mydb_abstract_db,
                                         "select * from opportunity_with_products_data")
        oppy_with_products = fetch(oppy_with_products, n=-1)
        
        live_inv_data = dbSendQuery(mydb_abstract_db,
                                    "select * from live_inventory_data")
        live_inv_data = fetch(live_inv_data, n=-1)
        
        cust_master_data = dbSendQuery(mydb_abstract_db,
                                       "select * from customer_data")
        cust_master_data = fetch(cust_master_data, n=-1)
        
        rate_card = dbSendQuery(mydb_abstract_db,
                                "select * from 20181024_ILL_rate_card")
  ...
  total_arc <- (bm_data$ILL_Port_MRC_Adjusted + bm_data$Last_Mile_Cost_MRC + bm_data$Discounted_CPE_MRC + bm_data$additional_IP_MRC)*(bm_data$opportunityTerm)
  total_nrc <- (bm_data$ILL_Port_NRC_Adjusted + bm_data$Last_Mile_Cost_NRC + bm_data$Discounted_CPE_NRC + as.numeric(bm_data$lm_nrc_mast_onrf) + as.numeric(bm_data$lm_nrc_mast_ofrf))
  bm_data$total_contract_value <- total_arc + total_nrc
  
  sell_with_commission <- function(commission, id, port_sp, port_lp, deal_reg, multi_year, doa, tcv)
  {
    ...
    return (output_frame[,c('prospect_id', 'tot_commision', 'manual')])
